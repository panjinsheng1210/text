...
class GoodsCat extends Common
{
    ...
    public function getList()
    {
        ...
        $data = $this->field('id, parent_id, name, type_id, sort, image_id,first_shopping')
            ->order([ 'sort' => 'asc'])
            ->select();
    ...
    protected function getTree($data)
    {
        ...
        foreach($data as $v)
        {
            if($v['parent_id'] == self::TOP_CLASS_PARENT_ID)
            {
                ...
                $new_data[$v['id']]['first_shopping'] = $v['first_shopping'];
            ...
            else
            {
                $new_data[$v['parent_id']]['subclass'][] = array(
                    ...
                    'image_id' => $v['image_id'],
                    'sort' => $v['sort'],
                    'first_shopping' => $v['first_shopping'],
            ...
            if(isset($v['subclass']) && count($v['subclass']) > 0)
            {
                foreach($v['subclass'] as $vv)
                {
                    $return_data[] = array(
                        ...
                        'sort' => $vv['sort'],
                        'first_shopping' => $vv['first_shopping'],
    ...
    public function getAllCat($id = false)
    {
        ...
        $data = $this->field('id, parent_id, name, sort, image_id, first_shopping')
            ->where($where)
            ->order('sort asc')
            ->select();
     ...
     * @throws \think\exception\DbException
     */
    public function getChildClass($first_shopping = false, $parent_id = self::TOP_CLASS_PARENT_ID)
    {
        $where[] = array('parent_id', 'eq', $parent_id);

        if ($first_shopping) {
            $where[]  = ['first_shopping', 'eq', 1];
        }
    ...
    {
        $where[] = ['id', 'eq', $id];
        $data = $this->field('id, name, parent_id, type_id, sort, image_id, first_shopping')
            ->where($where)
            ->find();
        if($data)
        {
