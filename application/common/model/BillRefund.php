...
class BillRefund extends Common
{
    ...
    public function toRefund($refund_id,$status,$payment_code="")
    {
        ...
        if($status == self::STATUS_REFUND){
            ...
            $result['msg'] = ($result['status'] == true) ? '退款单退款成功':$result['msg'];
     ...
     * @throws \think\exception\DbException
     */
    public function shopTableData($post)
    {
        if(isset($post['limit'])){
            $limit = $post['limit'];
        }else{
        ...
        $list = Db::table(config('database.prefix').'bill_refund')
            ->alias('br')
...
//        $data = $this->tableFormat($list->getCollection());         //返回的数据格式化，并渲染成table所需要的最终的显示数据类型
    ...
    protected function tableWhere($post)
    {
        ...
        if(isset($post['shop_id']) && $post['shop_id'] != ""){
            $where[] = ['o.shop_id', 'eq', $post['shop_id']];
        }
        $result['where'] = $where;
        $result['field'] = "*";
