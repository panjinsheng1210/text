...
class UserTocash extends Common
{
    ...
    public function tocash($user_id, $money,$remarks='')
    {
        $result = [
            'status' => false,
            'msg' => '',
        ...
        $bankcardsInfo = $userBankcardsModel->where(['user_id'=>$user_id,'is_default'=>1])->find();
        if(!$bankcardsInfo){
...
//        $data['account_bank'] = $bankcardsInfo['account_bank'];
    ...
    //用户撤销提现
    public function cancelTocash($id){
        $result = [
            'status' => false,
            'msg' => '',
            'data' => ''
    ...
    protected function tableWhere( $post )
    {
        ...
        if ( isset($post[ 'role' ]) && $post[ 'role' ] != "" ) {
            $where[] = [ 'role', 'eq', $post[ 'role' ] ];
        }
        $result[ 'where' ] = $where;
        $result[ 'field' ] = "*";
        $result[ 'order' ] = 'type asc';
        return $result;
    }
...
//            }
     ...
     */
    public function userToCashList($user_id, $page, $limit, $type='',$role=1,$shopId = false)
    {
        $result = [
            'status' => true,
            'msg' => '获取成功',
    ...
    public function delTocash($id){
        ...
        $addMoney = $ShopOtayonii->setShop($tocash['shop_id'],$tocash['money'] + $tocash['withdrawals'],4,3,'店铺提现删除增加'.$tocash['money'].'元')['status'];
        $delTocash = $this->where(['id'=>$id])->delete();
        if($addMoney && $delTocash){
            $result = [
                'status' => true,
                'msg' => '删除成功',
            ];
    ...
    // 店铺提现
    public function addTocash($user_id, $money,$bankcards_id,$shop_id,$remarks='')
    {
        $result = [
            'status' => false,
            'msg' => '提现成功',
        ...
        $re = $this->save($data);
        $ShopOtayonii = new ShopOtayonii();
        $status = $ShopOtayonii->setShop($shop_id,-$money,4,3,'店铺提现'.$money.'元')['status'];
        if($re && $status){
            $result['status'] = true;
            Db::commit();
        }else{
