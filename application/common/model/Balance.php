...
class Balance extends Common
{
    ...
    const TYPE_APPROPRIATE = 8;        // 公司拨款
    const TYPE_DEDUCTION = 9;          // 公司扣款
    const TYPE_OFFINE_PAY = 20;          //线下交易付款
    const TYPE_RECHARGE_PAY = 21;       //手机充值扣款
    const TYPE_TRANSFER = 12;           //转账
    /**
     * 充值
     */
     ...
     */
    public function change($user_id, $type, $money, $source_id = "", $cate_money = 0,$memo='')
    {
        $result = [
            'status' => false,
            'data' => [],
        ...
        if($money != 0)
        {
            ...
            (float)$money = $money;
            if($type != self::TYPE_ADMIN  && $type != self::TYPE_DEDUCTION  && $type!=self::TYPE_OFFINE_PAY && $type!=self::TYPE_TRANSFER)
            {
                //后台充值或调不改绝对值
                $money = abs($money);
            }
            ...
            if($type==self::TYPE_OFFINE_PAY || $type==self::TYPE_RECHARGE_PAY){
                $money = - $money;
            }
     ...
     * @return array|mixed
     */
    public function getMemo($type, $money, $source_id = "", $cate_money = '')
    {
        $result = [
            'status' => true,
            'data' => [],
        ...
        switch($type)
        {
            ...
            case self::TYPE_ADMIN:
                ...
                break;
            case self::TYPE_APPROPRIATE:
                $result['data'] = '公司拨款' . $money . '元';
                break;
            case self::TYPE_DEDUCTION:
                $result['data'] = '公司扣款' . $money . '元';
                break;
            case self::TYPE_OFFINE_PAY:
                $result['data'] = '线下交易付款' . $money . '元';
                break;
            case self::TYPE_RECHARGE_PAY:
                $result['data'] = '手机充值付款' . $money . '元';
                break;
            default:
    ...
    public function getInviteCommission($user_id)
    {
        ...
        {
            $return['data'] = sprintf("%.2f",$return['data']);
            $return['status'] = true;
            $return['msg'] = '获取成功';
        }
