...
class Goods extends Common
{
    ...
    protected function tableWhere($post)
    {
        ...
        if (session("shop.id")) {
            $where[] = ['shop_id', 'in',session("shop.id")];
        }
        if (isset($post['warn']) && $post['warn'] == "true") {
            ...
            $where[] = ['stock','lt',$goods_stocks_warn];
        }
    ...
    protected function tableFormat($list)
    {
        ...
        foreach($list as $key => $val){
            ...
            if(!empty($val['shop_id'])){
                $shop_name =getShopName($val['shop_id']);
            }else{
                $shop_name = '自营';
            }
            if($val['buy_type']==2){
                $list[$key]['coupon'] = ceil($list[$key]['price']);
                $list[$key]['price'] = '';
            }else{
...
//        return $this->belongsto('Shop','shop_id','id');
    ...
    public function changeStock($product_id, $type = 'order', $num = 0)
    {
        ...
        switch ($type) {
            case 'order': //下单
                $res = $productModel->where($where)->fetchSql()->setInc('freeze_stock', $num);
                break;
            case 'send': //发货
    ...
    public function staticGoods($baseFilter=[]){
        if(session("shop.id")){
            $baseFilter[] = ['shop_id', 'eq',session("shop.id")];
            $baseFilter1[] = ['shop_id', 'eq',session("shop.id")];
        }
        ...
        if(session("shop.id")) {
            ...
            $totalWarn = $this->where("(stock - freeze_stock) < " . $goods_stocks_warn . " and shop_id=$shop_id")->count('id');
        }else{
            $totalWarn = $this->where("(stock - freeze_stock) < " . $goods_stocks_warn)->count('id');
        }
        return [
            'totalGoods'=>$total,
    ...
    public function getShopCat($shop_id)
    {
        $return = [
            'status' => false,
            'msg' => '获取失败',
        ...
        $return['data'] = $this->with('goodsCat')->where($where)->field('distinct goods_cat_id')->select();
        if($return['data'])
        {
            $return['status'] = true;
            $return['msg'] = '获取成功';
