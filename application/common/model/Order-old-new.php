...
class Order extends Common
{
    ...
    protected function getListByWhere($input, $isPage = true)
    {
        ...
        if (!empty($input['pay_status'])) {
            ...
            if($input['pay_status'] == self::ALL_PENDING_PAYMENT){
                $where[] = ['o.parent_order_id','null'];
            }else{
                $where[] = ['o.parent_order_id','not null'];
            }
        }
        ...
        if(session("shop.id")) {
            $where[] = array('shop_id', 'eq', session("shop.id"));
        }
        if (!empty($input['date'])) {
        ...
        if ($isPage) {
            ...
            $data = $this->alias('o')
                ->field('o.order_id,o.shop_id, o.user_id, o.ctime, o.ship_mobile, o.ship_address, o.status, o.pay_status, o.ship_status, o.confirm, o.is_comment, o.order_amount, o.source, o.ship_area_id,o.ship_name, o.mark')
    ...
    public function getListFromAdmin($input, $isPage = true)
    {
        ...
        if (count($result['data']) > 0) {
            ...
            foreach ($result['data'] as $k => &$v) {
                ...
                if(!empty($v['shop_id'])){
                    $shop_name =getShopName($v['shop_id']);
                }else{
                    $shop_name = '自营';
                }
...
//            $whereOr = ['pay_status'=>1,'parent_order_id'=>'null'];
        }
        if (!empty($input['user_id'])) {
    ...
    public function getOrderStatusNum($input)
    {
        ...
        $data = [];
        foreach ($ids as $k => $v) {
            $data[$v] = $this->orderCount($v,$input['user_id']);
        }
        ...
        if ($isAfterSale) {
            ...
            $number              = $model->getUserAfterSalesNum($model::STATUS_WAITAUDIT);
...
//        }
    ...
    protected function getReverseStatus($status, $table_name = '')
    {
        ...
        if($status == self::ALL_PENDING_PAYMENT){
            $where[] = [$table_name.'parent_order_id','null'];
        }elseif ($status == 0){

        }else{
            $where[] = [$table_name.'parent_order_id','not null'];
        }
        return $where;
    }
        ...
        try{
            ...
            if(isset($orderInfo) && !empty($orderInfo)){
                foreach ($orderInfo as $key=>$val){
                    ...
                    $addPoint += $val['goods_amount'] * ($val['shop_type']==1?$prizeConfig['enterprise_point_ratio']:$prizeConfig['personal_point_ratio'])/100;
                    $addOtayonii += $val['goods_amount'] * ($val['shop_type']==1?$prizeConfig['enterprise_otayonii_ratio']:$prizeConfig['personal_otayonii_ratio'])/100;
                }
            }
            // 消费者添加积分明细
            ...
            $parentData  = (new User())->getParentAndMoney(1,3);
            if(isset($parentData) && !empty($parentData)){
                ...
                foreach ($parentData as $key=>$val){
                    ...
                    if(in_array($key,[1,2])){
                        ...
                        $memo = '推荐奖增加'.$money*$val['ratio'].'元,来源用户:'.$userInfo->mobile;
                        $blanceModel->change($val['id'],10,$money*$val['ratio'],0,0,$memo);
                        // 添加奖金明细
                        (new PrizeRecord())->addPrizeRecord(['currency'=>$money*$val['ratio'],'user_id'=>$val['id'],'source_id'=>$consumer_id,'type'=>1,'ctime'=>time()]);
                    }else{
                        break;
                    }
                }
...
//                    $currentRatio = $ratio['rangeRatio'][$val['grade']];
                    ...
                    if($currentRatio>$previousRatio){
                        $blanceModel = new Balance();
                        $memo = '极差奖增加'.$money*($currentRatio-$previousRatio).'元,来源用户:'.$userInfo->mobile;
                        $blanceModel->change($val['id'],14,$money*($currentRatio-$previousRatio),0,0,$memo);
                        (new PrizeRecord())->addPrizeRecord(['currency'=>$money*($currentRatio-$previousRatio),'user_id'=>$val['id'],'source_id'=>$consumer_id,'type'=>2,'ctime'=>time()]);
