                ...
                switch($k){
                    case 'order_all':
                        $where = "
                            ...
                            and ((pay_status = 1 and parent_order_id is null) or ( pay_status > 1 and not (parent_order_id is null)) )
                        ";
                    ...
                    case 'order_payed':
                        $where = "
                            ...
                            and pay_status > 1
                            and not (parent_order_id is null) 
                        ";
                ...
                switch($k){
                    case 'payments_all':
                        $where = "
                            ...
                            and status = 2
                            and not (parent_payment_id is null)
...
//            $select = "select sum(oi.nums) as nums,sum(oi.amount) as amount,oi.sn,oi.name,oi.image_url,oi.addon ";
            $select = "select sum(oi.nums) as nums,sum(oi.amount) as amount,oi.sn ";
//            $select = "select sum(oi.nums) as nums,sum(oi.amount) as amount ";
            $limit_sql = " limit ".($page-1)*$limit.",".$limit;
            $resultData = [];
            $orderInfo = $orderItemModel->query($select.$sql.$limit_sql);
            if(isset($orderInfo) && !empty($orderInfo)){
                foreach ($orderInfo as $key=>&$val){
                    $OrderItemsInfo  = OrderItems::where(['sn'=>$val['sn']])->find();
                    $val['name']      = $OrderItemsInfo['name'];
                    $val['image_url'] = $OrderItemsInfo['image_url'];
                    $val['addon']     = $OrderItemsInfo['addon'];
                    $resultData[] = $val;
                }
            }
...
//                    case 'order_all':
//                        $ptime = "
//                            and o.payment_time >" .$date_re['data']['start'].  "
//                            and o.payment_time <" .$date_re['data']['end'].  "
//                            ";
//                        $ctime = "
//                            and o.ctime >" .$date_re['data']['start'].  "
//                            and o.ctime <" .$date_re['data']['end'].  "
//                            ";
//                        $where = "
//                            and ((pay_status = 1 and parent_order_id is null $ctime) or ( pay_status > 1 and not (parent_order_id is null) $ptime) )
//                        ";
//                        $data = $this->shopMark($where);
//                        break;
//                    case 'order_nopay':
//                        $where = "
//                            and ctime >" .$date_re['data']['start'].  "
...
//                }
//            }
//            $this->option['xAxis']['data'] =array_column($this->shopMark($where)['data'],'name');
            //组装数据列表用于table里使用
            $table = [];
            $shop = \app\common\model\Shop::where(['is_verify'=>1])->select()->toArray();
            if(isset($shop) && !empty($shop)){
                foreach ($shop as $key=>$val){
                    $item['name'] = $val['name'];
                    $all = $this->getOrderMoney(0,$val['id']);
                    $item['order_all_val']   = $all['money'] ? $all['money']:0;
                    $item['order_all_num']   = $all['num'] ?$all['num']:0;
                    $nopay = $this->getOrderMoney(1,$val['id']);
                    $item['order_nopay_val'] = $nopay['money']?$nopay['money']:0;
                    $item['order_nopay_num'] = $nopay['num']?$nopay['num']:0;
                    $payed = $this->getOrderMoney(2,$val['id']);
                    $item['order_payed_val'] = $payed['money']?$payed['money']:0;
                    $item['order_payed_num'] = $payed['num']?$payed['num']:0;
                    $table[] = $item;
                }
...
//            }
    ...
    public function shopMark($where){
        $result = [
            'status' => false,
            'data'   => [],
            'msg'    => '',
        ];
        $sql = "SELECT ifnull(sum(o.order_amount),0) as val,o.shop_id,s.name 
                FROM jshop_order as o,
                jshop_shop as s
    ...
    public function getOrderMoney($status,$shop_id){
        $payWhere = ' where type =3 and shop_id = '.$shop_id;
        $orderWhere = ' where shop_id = '.$shop_id;
        switch ($status){
            case 1:
                $orderWhere .= " and pay_status = 1";
                break;
            case 2:
                $orderWhere .= " and pay_status > 1";
                break;
        ...
        $orderSql = "
            SELECT ifnull(sum(order_amount),0) as money,ifnull(count(*),0) as num
            FROM  jshop_order
            $orderWhere
        ";
        switch ($status){
            case 1:
                ...
                break;
            case 2:
                $payWhere .= " and status = 2";
                break;
            case 0:
                $payWhere .= " and status in (1,2)";
                break;
        }
        $paySql = "
            SELECT ifnull(sum(money),0) as money,ifnull(count(*),0) as num
            FROM  jshop_bill_payments
            $payWhere
        ";
        $orderData = Db::query($orderSql);
        $payData   = Db::query($paySql);
        $data['money'] = $orderData[0]['money'] + $payData[0]['money'];
        $data['num']   = $orderData[0]['num'] + $payData[0]['num'];
        return $data;
    }
