...
class Goods extends Manage
{
    ...
    private function _common()
    {
        //商家列表
        $shopModel = new Shop();
        $shopList = $shopModel->getCertificationList();
    ...
    private function checkGoodsInfo($isEdit = false)
    {
        ...
        $data['goods']['shop_id']       = input('post.shop_list','');
        $data['goods']['goods_cat_id']  = input('post.goods_cat_id','0');
        if ($data['goods']['goods_cat_id']) {
            $goodsCat = GoodsCat::get($data['goods']['goods_cat_id']);
            $data['goods']['first_shopping'] = $goodsCat->first_shopping;
        }
        $data['goods']['goods_type_id'] = input('post.goods_type_id', 0);
        $data['goods']['buy_type']      = input('post.buy_type', 1);
        ...
        $data['goods']['point']        = input('post.goods.point', 0);
        $open_spec                     = input('post.open_spec', 0);
