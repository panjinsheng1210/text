...
class Report extends \app\common\controller\Shop
{
    ...
    private function refundMark($date_arr,$where,$join_val){
        ...
        $orderId = model('common/Order')->field('order_id')->where("shop_id=$this->shopId")->select();
        if(!$orderId->isEmpty()) {
            $orderIds = array_column($orderId->toArray(), 'order_id');
