...
{
    $shop_obj = new \app\common\model\Shop();
    $shops = $shop_obj->field('name')->where('id', 'eq', $id)->find();
    if (isset($shops['name'])) {
        return $shops['name'];
    }
    return '';
 ...
 * 输出调式信息到chrome控制台
 */
function d($msg,$tag='debug')
{
    PhpConsole\Connector::getInstance()->getDebugDispatcher()->dispatchDebug($msg, $tag);
}
...
function getMobileInfo($mobile){
    ...
    if (!preg_match("/^1[34578]\d{9}$/", $mobile)) {
        ...
        return $phone_array['data'][0]['prov'].mb_substr($phone_array['data'][0]['type'],2);
    }
}