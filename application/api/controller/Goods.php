...
//        }
    ...
    public function getList()
    {
        ...
        if (input('?param.where')) {
            ...
            if (isset($postWhere['buy_type'])) {
                $where[] = ['g.buy_type', 'eq', '2'];
            }
            if (isset($postWhere['shop_id'])) {
                $where[] = ['g.shop_id', 'eq', $postWhere['shop_id']];
            }
            //品牌筛选
    ...
    public function getDetail()
    {
        ...
        if ($returnGoods['status']) {
            if($returnGoods['data']['shop_id']){
                $shopInfo = \app\common\model\Shop::get($returnGoods['data']['shop_id']);
                if($shopInfo['logo']){
                    $shopInfo['logo'] = _sImage($shopInfo['logo']);
                }
                $returnGoods['data']['shop_info'] = $shopInfo ;

            }else{
                $returnGoods['data']['shop_info'] = [];
            }
            $return_data ['msg']  = '查询成功';
