...
class Store extends Api
{
    ...
    public function getStoreQRCode(){
        //根据用户id去取商户id
        $model = new Shop();
        $data = $model->where("user_id=$this->userId and is_verify = 1")->find();
        $shop_id = isset($data['id'])?$data['id']:0;
        if(!$shop_id){
            $return = [
                'status' => false,
                'msg' => '你没有符合条件的店铺',
        ...
        //根据shop_id把logo取出来
        $imageModel = new ShopImages();
        $logoPath = $imageModel->returnLogoPath($shop_id);
        $page = "pages/member/payment/payment?shop_id=$shop_id";
        $wx = new Wx();
        $wx_appid = getSetting('wx_appid');
        $wx_app_secret = getSetting('wx_app_secret');
        $accessToken = $wx->getAccessToken($wx_appid, $wx_app_secret);
        ...
        {
            $style['width'] = 300;
            return $wx->getStoreQRCode($accessToken, $page, $shop_id,$logoPath,$style, $wx_appid);
        }
        else
        {
            return $return = [
