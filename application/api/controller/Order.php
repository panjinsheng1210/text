...
//        $formId = Request::param('formId', false);
        $formId = Request::param('formId', 'the formId is a mock one');
        $tax['tax_type'] = Request::param('tax_type', 1);
        $tax['tax_name'] = Request::param('tax_name', '本次订单不需要发票');
        $tax['tax_code'] = Request::param('tax_code', '');
        $isCoupon = Request::param('is_coupon', 0);
        $model = new orderModel();
        return $model->toAdd($this->userId, $cart_ids, $uship_id, $memo, $area_id, $point, $coupon_code, $formId, $receipt_type, $store_id, $lading_name, $lading_mobile,$source,$tax,$isCoupon);
    }
