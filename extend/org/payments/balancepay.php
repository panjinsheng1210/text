...
class balancepay implements Payment
{
    ...
    public function pay($paymentInfo,$order_id =0)
    {
        ...
        if($paymentInfo['type']==3){
            $res = $balance->change($paymentInfo['user_id'], $balance::TYPE_OFFINE_PAY, $paymentInfo['money'], $paymentInfo['payment_id']);
        }elseif($paymentInfo['type']==4){
            $res = $balance->change($paymentInfo['user_id'], $balance::TYPE_RECHARGE_PAY, $paymentInfo['money'], $paymentInfo['payment_id']);
        }else{
...
//            $addPrize  = (new Order())->settlementBonus($paymentInfo['money'],$paymentInfo['user_id'],$order_id);
        }elseif ($paymentInfo['type'] ==3 ){
//            $addPrize  = (new Order())->offlineBonus($paymentInfo['money'],$paymentInfo['user_id'],$paymentInfo['shop_id']);
        }elseif ($paymentInfo['type'] ==4 ){
            //手机充值
            $result1 = (new Recharge())->recharge($paymentInfo);
            if(!$result1['status']){
                $billPaymentModel->where(['payment_id' => $paymentInfo['payment_id']])->data(array("status"=>3))->update();
            }
            return $result1;
        }
