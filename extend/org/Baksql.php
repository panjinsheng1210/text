...
//        $config["sqlbakname"]=date("YmdHis",time()).".sql";//默认保存文件
        $config["sqlbakname"]=$config['database']
            . '_' . date('Y-m-d_H_i_s',time()) . '_'
            .  hash('md5',rand(1,999) . time()) .  ".sql";
        $this->config = $config;
...
//                    $vs = array_map("mysql_real_escape_string",$v);
                    $v = str_replace("'","\'",$v);
                    $dataSql .= "'{$v}',";
                }
    ...
    private function writeToFile($tables = array(), $ddl = array(), $data = array())
    {
        ...
        if(file_put_contents($this->config['path'].$this->config['sqlbakname'], $str)){
            $result = ['success'=>true,'msg'=>'备份成功!花费时间' . round(microtime(true) - $this->begin,2) . 'ms','data'=>$this->config['sqlbakname']];
        }else{
            $result = ['success'=>false,'msg'=>'备份失败!'];
        }
        return $result;
    }
    /**
    ...
    public function restore($filename = '')
    {
        ...
        else
        {
            ...
            try
            {
                ...
                return ['status'=>true,'msg'=>'还原成功!花费时间', round(microtime(true) - $this->begin,2) . 'ms'];
